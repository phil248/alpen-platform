...
def main() -> int:
    ...
    if not out_lines:
        ...
        if not args.no_header:
            ...
            print()
        print("_All clean — no high-severity VoC signals, no stuck high-value deals, no at-risk engagements, no overdue status reports._")
        return 0

    if not args.no_header and not args.section:
        print("## 8c. Alpen Platform")
        print()
        print("_Pulled from `~/.local/state/alpen/sqlite/{voc-signals,leads,engagements}.db`. Source-of-truth markdown lives in `${VAULT}/Plaud-Recordings/` and `Cognitive-Capital-Group/Opportunities/`. Refreshed daily at 06:15 by `alpen-regenerate-all.sh`._")
        print()
