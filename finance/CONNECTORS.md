...
Plugin files use `~~category` as a placeholder for whatever tool the user connects in that category. For example, `SQLite + sqlite-vec warehouse` might mean Snowflake, BigQuery, or any other warehouse with an MCP server.
...
|----------|-------------|-----------------|---------------|
| Data warehouse | `SQLite + sqlite-vec warehouse` | Snowflake\*, Databricks\*, BigQuery | Redshift, PostgreSQL |
