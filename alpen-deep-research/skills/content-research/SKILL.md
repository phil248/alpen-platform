...
8. **Iterate ALL name variants (NON-NEGOTIABLE for person-targeted searches; tightened 2026-05-07 audit findings #3, #13).** When searching for a person's content (publications, talks, podcasts, op-eds, media mentions), you MUST issue separate searches for EVERY entry in `person.name_variants` — including informal/colloquial forms (e.g., "Sandi", "Sandy", first-name-only, married-name-only). This applies to ALL discovery surfaces — INCLUDING ORCID/OpenAlex/PubMed bulk queries, NOT just free-text/news APIs. The previous-rule scope (free-text only) is what allowed CBH Phase 1 to plateau at 146/199 ORCID-listed Sandra works: ORCID was queried by canonical_name only, the OpenAlex `display_name.search:` fallback was never iterated under "Sandi" / "S.B." variants. Authoritative-source rule: when a person has an `orcid_id`, ORCID `/works` is the AUTHORITATIVE per-person publication count and OpenAlex queries MUST use `author.orcid:<id>`. When ORCID is unknown, OpenAlex MUST be queried as `author.display_name.search:<variant>` for EVERY variant and results merged. Search engines and APIs do NOT reliably handle name variants automatically; "Sandra Chapman" and "Sandi Chapman" return DIFFERENT result sets on Google, News API, Listen Notes, YouTube, and most other sources. Real bug: CBH Phase 1 Stage 4 searched "Sandra Chapman" only and found 15 speaking engagements; targeted Sandi-variant searches in Stage 4.5 surfaced additional content (local Dallas press, podcast intros, LinkedIn re-shares). Cost: 3-5x more search calls but typically 30-50% more content discovered. Always run variant searches; mark `source_of_record._variant_used` per record to track which variant produced each discovery.

9. **Stage 3 not done until per-person count reconciles with ORCID.** (added 2026-05-07; audit finding #3). For every seed person with `orcid_id`, the post-discovery persisted-publication-count for that person MUST be >= ORCID(works/count) - 5 tolerance. If short, the person is flagged for manual review and the orchestrator's Stage 3.5 truth-check will fail loud. Do not return success for `target=person_publications` if you know your corpus is below the ORCID floor.

10. **One file per discovered entity.** (added 2026-05-07; today's NEW finding #13). Every discovered record returned to the orchestrator must be intended as ONE file when persisted. Never bundle 10 publications into a roll-up record with a nested `publications: [...]` list, never return a "list-of-records" record. memory-orchestration op=persist_batch will reject roll-ups; respect that contract upstream.
...
2. **OpenAlex enrichment for ORCID-listed works** (only fetch what ORCID didn't provide):
   ...
   **OA recovery via OpenAlex `open_access.oa_url`** (added 2026-05-07; audit finding #7). When the OpenAlex record has `open_access.is_oa: true` and a populated `open_access.oa_url`, copy that URL into `secondary_urls` AND prefer it over the publisher URL when populating `pdf_url` (override null pdf_urls). On 2026-05-07, Phase 2 recovered 17 PDFs that Phase 1 missed by querying this field; content-acquisition's Tier-1 flow now retries against `secondary_urls[0]` when the primary publisher URL is paywalled. Don't drop OA mirrors.
...
7. **Amplification vs media-mention classification** (added 2026-05-07; audit finding #4). When fetching a CBH-owned `/article/`, `/news/`, or `/blog/` page that contains an OUTBOUND link to a non-CBH outlet (and where the page is editorially summarizing/excerpting that outlet's coverage), classify the entity as `amplification` (NOT `media-mention`). Persist under `amplifications/<slug>.json` with the schema in `_shared/entity-schemas/amplification.yaml`. Record BOTH URLs: `url` = the CBH-owned page (source-of-record), `amplified_external_url` = the original outlet URL. The original third-party piece is still a `media-mention` if independently surfaced via News API; the CBH editorial choice to amplify is a separate entity. Without this distinction, attribution analytics conflate "we covered this" with "they covered us." Persist amplifications and media-mentions in DIFFERENT folders.
